...
workflow {
    ...
    if (!fasta || !fasta.exists()) {
        exit 1, "ERROR: fasta reference file not found: " + 
                "${params.genomes[params.genome]?.fasta ?: 'Not specified'}"
    }
    if (!fai || !fai.exists()) {
        exit 1, "ERROR: fasta.fai file not found: " + 
                "${params.genomes[params.genome]?.fai ?: 'Not specified'}"
    }
    if (!dict || !dict.exists()) {
        exit 1, "ERROR: Dict file not found: " + 
                "${params.genomes[params.genome]?.dict ?: 'Not specified'}"
    }
    
    // Check for other required files
    if (!intervals || !intervals.exists()) {
        log.warn "WARNING: Intervals file not found: " + 
                "${params.genomes[params.genome]?.intervals ?: 'Not specified'}"
    }
    if (!dbsnp || !dbsnp.exists()) {
        log.warn "WARNING: dbSNP file not found: " + 
                "${params.genomes[params.genome]?.dbsnp ?: 'Not specified'}"
    }
    
    if (!bait_intervals || !bait_intervals.exists() || 
        !target_intervals || !target_intervals.exists()) {
        log.warn "WARNING: Bait or target intervals not found. " +
            "Hybrid metrics may fail."
    }
    
    // Create BWA index channel with all index files
    fasta_index = Channel
        .fromPath("${fasta}.{amb,ann,bwt,pac,sa}", checkIfExists: true)
        .ifEmpty { 
